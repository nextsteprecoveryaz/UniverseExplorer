...
async function openCloudConnection(){
  ...
  $('#cloud-key-form').onsubmit=async e=>{
    ...
    await busy($('#cloud-key-save'),'Checking access…',async()=>{
      cloudConnection=await jsonPost('/api/cloud/connect',{api_key:key,remember});updateEnhancementControls();closeModal();toast('OpenAI connected. Choose cloud enhancement when you want to send an image.');
    });
  };
