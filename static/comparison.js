...
function initComparison(){
  ...
  const options=state.config.surveys.map(s=>`<option value="${esc(s.id)}">${esc(s.name)}</option>`).join('');
  $('#compare-left').innerHTML=options;$('#compare-right').innerHTML=options;$('#compare-left').value='webb-color';$('#compare-right').value='hubble-color';
