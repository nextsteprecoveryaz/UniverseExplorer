...
function renderTourTemplates(){
  ...
  $('#cefca-guided-tours').innerHTML=cefca.map(({r,i})=>`<article class="cefca-tour-card"><figure><img src="${esc(r.cover_url||'/assets/cefca-virgo.jpg')}" alt="Virgo Cluster mosaic from JAST80 / T80Cam" loading="lazy"><figcaption>Image: <a href="${esc(r.source_url)}" target="_blank" rel="noopener">CEFCA Foundation ↗</a></figcaption></figure><div class="cefca-tour-body"><div class="cefca-tour-tags"><span>${r.stops.length} sky stops</span><span>JAST80 / T80Cam</span><span>${routeTime(r.stops.reduce((n,s)=>n+s.travel+s.hold,0))} without narration</span></div><h3>${esc(r.title)}</h3><p>${esc(r.description)}</p><p class="cefca-tour-language">Researched English stories · Original guide in Spanish</p><div class="cefca-tour-actions"><button class="button primary" data-template-start="${i}">Start Virgo tour ↗</button><button class="button" data-template-copy="${i}">Customize route</button><a href="${esc(r.source_url)}" target="_blank" rel="noopener">Original Spanish tour ↗</a></div></div></article>`).join('');
