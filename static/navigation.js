...
function navigationStop(media){return {title:media.title||media.name,ra:media.ra,dec:media.dec,fov:media.fov||.18,roll:0,survey:media.survey||'optical',projection:'AIT',notes:'',travel:4,hold:15,source:{kind:media.kind,id:String(media.id)}};}
...
async function inspectNavigationMedia(m){
  ...
  try{
    ...
    else if(m.kind==='cefca')modal(m.title,`<p>${esc(m.description)}</p><p class="small muted">${esc(m.credit)} · JAST80 / T80Cam</p><p>RA ${Number(m.ra).toFixed(6)}° · Dec ${Number(m.dec).toFixed(6)}°</p><a class="button" href="${esc(m.source_url)}" target="_blank" rel="noopener">Read the original Spanish guide ↗</a>`);
...
async function addCurrentRouteStop(){
  ...
  const p=skyView();r.stops.push({...p,title:'Waypoint '+(r.stops.length+1),notes:'',travel:4,hold:12,source:null});r.media.push(null);markRouteDirty();renderRouteEditor();await saveEditedRoute();toast('Current view added to '+r.title);
}
...
function renderRouteEditor(){
  ...
  $$('[data-stop-survey]').forEach(el=>el.onchange=()=>{r.stops[Number(el.dataset.stopSurvey)].survey=el.value;markRouteDirty();});
  ...
  $$('[data-stop-visit]').forEach(el=>el.onclick=()=>{const s=r.stops[Number(el.dataset.stopVisit)];flyTo({name:s.title,...s});});
  renderSavedRoutes();
}
...
function renderTourTemplates(){
  ...
  $('#cefca-guided-tours').innerHTML=cefca.map(({r,i})=>`<article class="cefca-tour-card"><figure><img src="${esc(r.cover_url||'/assets/cefca-virgo.jpg')}" alt="Virgo Cluster mosaic from JAST80 / T80Cam" loading="lazy"><figcaption>Image: <a href="${esc(r.source_url)}" target="_blank" rel="noopener">CEFCA Foundation ↗</a></figcaption></figure><div class="cefca-tour-body"><div class="cefca-tour-tags"><span>${r.stops.length} sky stops</span><span>JAST80 / T80Cam</span><span>${routeTime(r.stops.reduce((n,s)=>n+s.travel+s.hold,0))} at 1×</span></div><h3>${esc(r.title)}</h3><p>${esc(r.description)}</p><p class="cefca-tour-language">English navigation · Original guide in Spanish</p><div class="cefca-tour-actions"><button class="button primary" data-template-start="${i}">Start Virgo tour ↗</button><button class="button" data-template-copy="${i}">Customize route</button><a href="${esc(r.source_url)}" target="_blank" rel="noopener">Original Spanish tour ↗</a></div></div></article>`).join('');
  $('#guided-tours').innerHTML=rows.filter(({r})=>r.collection!=='cefca').map(({r,i})=>{const image=r.media.find(m=>m?.preview_url)?.preview_url||'/assets/pillars.jpg';return `<article class="tour-card"><img src="${esc(image)}" alt="${esc(r.media.find(m=>m?.preview_url)?.title||'Pillars of Creation')}" loading="lazy"><div class="tour-card-shade"></div><div class="tour-card-body"><span class="overline">${r.stops.length} SOURCED STOPS · ${routeTime(r.stops.reduce((n,s)=>n+s.travel+s.hold,0))}</span><h2>${esc(r.title)}</h2><p>${esc(r.description)}</p><button class="button primary" data-template-start="${i}">Start guided tour ↗</button><button class="text-button" data-template-copy="${i}">Customize</button></div></article>`;}).join('');
...
async function nearbyTour(){
  ...
  const r=await jsonPost('/api/navigation/routes',{title:'Nearby observations · '+new Date().toLocaleDateString(),description:'A route through distinct nearby MAST pointings. Real previews are displayed beside the sky context; footprint checks do not imply HiPS mosaic coverage.',kind:'waypoints',stops,track:[]});
