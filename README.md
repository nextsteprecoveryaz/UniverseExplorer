...
**Preload flight & tour views** warms survey tiles about one second ahead of the current flight velocity, at most once every two seconds. It also requests rendered views for the next two waypoint stops; the current stop can use its prepared view while detailed survey tiles arrive. Preloading is optional and pauses in cached-only mode. Transfers still depend on archive and CDS response times.
...
The atlas upgrade passed **57 Python and 29 JavaScript tests** on this installation. JavaScript checks: `node --test tests/test_atlas_ui.cjs tests/test_flight.cjs tests/test_flight_controls.cjs tests/test_route_timeline.cjs tests/test_route_player.cjs`. With the app running, `.venv\Scripts\python.exe verify_atlas.py` verifies real original-FITS cutouts, saved view files, the spatial index and cached-only behavior. Results are in `data/verification/atlas-api.json` and `atlas-ui.json`.
