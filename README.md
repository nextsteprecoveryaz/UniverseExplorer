...
**CEFCA: Virgo Cluster** is available at the top of **Tours & routes**. Start its 23-stop itinerary or choose **Customize route** to save your own version. The published stop order, coordinates and fields follow CEFCA's [original Spanish tour](https://www.cefca.es/divulgacion/tour_cumulo_virgo). Original English stories explain the science behind each stop, with primary references under **Story sources** in the player and **About this stop**. The scripts and research links are bundled in `tour_content/`; saved routes resolve the latest story when reopened without changing your waypoints or notes. Stories distinguish known object facts from general explanations, and the survey view may not show features revealed at other wavelengths or resolutions. The CEFCA site currently lists this one complete tour. Its imagery and original cover remain credited to CEFCA Foundation. This integration uses our existing Aladin 3 viewer and route player.
