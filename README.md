...
**Explore the sky → Pixel mapping** opens the same levels, color maps, stretch, contrast, brightness and red/yellow/green/blue controls beside the main sky map. Settings are saved independently for each lens, including SDSS, and separately from Telescope Live. Closing the panel keeps your adjustments; **Show original** temporarily bypasses them and **Reset display** restores neutral controls. The sidebar's **Display stretch** stays synchronized. **Save PNG** and **Open in Image Lab** include the displayed adjustments and the selected survey's source credit. FITS survey levels are mapped to their native intensity range; these remain display controls rather than calibrated measurements.
...
2. Use **Visible**, **Near-IR**, or the wavelength selector to switch survey datasets. Hydrogen, nitrogen, molecular-hydrogen-band, and dust views have different resolutions and coverage. Released Hubble/Webb photo composites and the sparse O III/S II layers are excluded from map lenses. Their image and archive tools remain available, including SDSS MaNGA measurements.
...
**CEFCA: Virgo Cluster** is available at the top of **Tours & routes**. Start its 23-stop itinerary or customize it. Stop positions and order follow CEFCA's [original Spanish guide](https://www.cefca.es/divulgacion/tour_cumulo_virgo), while the map uses your chosen full-sky survey. Original English stories explain each stop, with primary references under **Story sources** and **About this stop**. The scripts and links live in `tour_content/`; reopening a saved route picks up the latest story while preserving your stops and notes. CEFCA is credited for the itinerary; the current survey's image credit appears on the map.

Choose **Sky survey for all tours** or **Tour sky** in the player: Visible sky (DSS2), 2MASS near-infrared, H-alpha, or WISE 12-micron dust. Visible sky is the initial default; your selection is remembered for the whole journey, including travel between stops, resumed tours, imported routes, and recorded paths. H-alpha is a coarse diffuse-emission map and WISE dust removes compact sources; use Visible or 2MASS to see stars and galaxies. Full-sky surveys can still contain masked or missing measurements. The original CEFCA mosaic remains available for manual exploration outside tours.
...
Open **Tours & routes** for sky landmarks, stellar nurseries, Webb galaxy targets, star-forming regions, located NASA releases, and recent public Hubble and Webb fields. Publisher and archive records supply target positions, narration and source links. Tour availability depends on the local indexes. **Update images** refreshes those indexes; reopening Tours & routes rebuilds the suggested itineraries. Starting a tour saves a local copy you can customize.

Tours display the continuous survey map with text, narration, coordinates, and source links. Tour cards and stop panels do not load publisher photos, JPEG previews or source videos. **About this stop** opens the text and research links. Starting a tour clears existing science images, research mosaics and SDSS overlays; delayed image requests cannot add them back during the tour. Automatic and manual science overlays remain paused while the tour guide is open.

These limits apply only to the tour view. **SDSS galaxies**, all main telescope/wavelength choices, and the other image workspaces remain available. Opening another workspace, visiting another destination, or choosing a main survey leaves the tour view and releases its map layers. Returning to Explore does not restart the tour. Explicitly resuming restores the tour's selected survey and any compatible downloaded views.

Playback follows great-circle paths with smooth zoom and rotation, staying on the selected full-sky survey throughout. Use **Pause**, **Previous/Next**, the position slider, and playback speed controls. Changing **Tour sky** keeps the same position and narration; it changes the display choice without rewriting saved camera paths or personal notes. **Take controls** switches to manual flight. Space pauses/resumes outside editable fields; Escape pauses. Steering, opening a dialog, changing workspace, or losing window focus pauses playback. A stop waits for survey tiles and narration before advancing, and explains prolonged tile-loading failures.
...
For your own flight, choose **Record a flight** or **Fly → Record flight**. It samples RA, Dec, zoom, rotation, projection, and survey twice per second, for up to 30 minutes or 3,600 samples. Recording pauses while the window is unfocused, another workspace/dialog is open, or the recording pause control is selected. Steering can be stopped while recording continues to capture a stationary view. Autosave runs approximately every five recorded seconds; **Stop & save** flushes the final samples and opens the saved route. **Backup** exports the in-memory recording if saving fails. A sudden browser/process crash can lose samples since the last successful autosave. Replay interpolates the recorded camera positions using your selected full-sky tour survey; it does not store a video or cache all image tiles.
...
**Auto-load science cutouts** is enabled in the sidebar. Below a 1-degree field, pausing over a supported fixed-target archive footprint requests a small section of its public science FITS. Choose Hubble, Webb, or both; adjust **Science layer opacity**, use **Load here** to retry, and open **Source details** for the observation, filter, native pixel scale, download size, processing and hashes. A delayed response cannot move the camera or insert a cutout after you have left its field. These image overlays are suppressed while the tour guide is open.
...
**Prioritize visible map detail** is enabled by default and remembered in your browser. It pauses the offscreen flight preview and waits for visible tiles before requesting automatic science cutouts or upcoming tour previews. Manual **Load here** remains available outside tours, and downloaded full-sky tour views can still be played. This gives the imagery you are looking at first access to download slots without lowering its resolution.
...
Open **Downloads & cache**, select a saved waypoint tour and a full-sky survey, then choose **Download tour views**. Each stop saves a registered 768 by 768 rendering of that survey, captions and the route revision; publisher photos are not downloaded. Packs for different surveys are kept separately. **Play downloaded tour** uses the saved camera path and current tour survey. A saved rendering is shown only when it matches that survey, so old sparse-image packs cannot add unrelated imagery. Partial downloads remain visible; cancelling skips remaining views after the current bounded transfer. Removing a download releases cache protection and leaves the route intact.
...
For **Visible sky / DSS2**, **2MASS near-infrared**, and all four **SDSS optical lenses**, a temporary connection failure or server error automatically tries the same tile on a verified alternate CDS server. The app briefly prefers the responding server so subsequent detail tiles do not repeat the same timeout. Shorter connection attempts apply only to these surveys with verified mirrors; other lenses retain their original connection and transfer timeouts. The canonical survey source stays unchanged, and the actual retrieval URL is recorded in cache metadata and the `X-Atlas-Retrieval-URL` response header. Missing-coverage responses are preserved. SDSS covers part of the sky; M51 is a covered field for checking its lenses. If a view was already stuck with blurry tiles before a connection recovered, reload the page to request its detail again. [CDS HiPS server registry](https://aladin.cds.unistra.fr/java/nph-aladin.pl?frame=aladinHpxList)
...
Choose **Standard 2**, **High Fidelity 2**, or **High Fidelity 3** for precision upscaling; **Bloom 2**, **Wonder 3.5**, and **Recover 3** provide creative or generative alternatives. Each model shows only its supported controls. Bloom includes creativity, color preservation, an optional prompt and grain; Wonder offers enhancement strength and grain; Recover offers recovery strength, creativity, texture and a prompt. Precision models offer sharpening and denoising with automatic settings, and High Fidelity 3 adds recovery strength and enhancement opacity. Creative models can invent structures, so their results remain labeled visualizations and never enter scientific source detection. [Topaz model documentation](https://developer.topazlabs.com/llms.txt)

The app sends the full raster original or a stretched FITS display image, with a 25-million-pixel output limit. Results retain the selected model and settings in provenance and join the comparison picker. Original files remain unchanged. Failed submissions are not automatically retried; a saved pending job is resumed only with the same model and settings. Keys can be kept in backend memory or encrypted for your Windows account using DPAPI; `TOPAZ_API_KEY` is also supported. Keys are never returned to the browser or committed to Git.
...
Open any saved image in **Image Lab → Create 3D**. **Astronomy scene** creates a local image-textured relief with source-derived star layers, adjustable depth and gentle orbit. **TRELLIS.2** generates an AI mesh with inferred unseen surfaces when its optional local GPU runtime is ready. Both preserve the original, save a self-contained GLB for Blender or other glTF viewers, and export a provenance JSON with source hashes and settings. Original FITS inputs use the selected display stretch; enhanced inputs use the latest saved cloud result, otherwise the local enhancement. Save pixel-mapping adjustments as a new image first to include them.

The 3D workspace supports orbit, zoom, pan, reset, automatic rotation, cancellation and reopening saved models. All geometry is an interpretation; brightness does not measure distance. Astronomy scenes retain the source's front view and restrict orbit because a single exposure cannot supply the hidden side. TRELLIS can work poorly on diffuse nebulae and crowded star fields, since these are not isolated solid objects.

Models, input snapshots and provenance are saved in `data/image-3d/`. The viewer uses locally installed [Three.js](https://github.com/mrdoob/three.js) 0.180.0 (MIT); `setup_three.py` verifies pinned checksums. The optional [TRELLIS.2](https://github.com/microsoft/TRELLIS.2) runtime runs in a separate WSL environment and reads `data/trellis-runtime.json`. It requires model downloads and approved access to [DINOv3](https://huggingface.co/facebook/dinov3-vitl16-pretrain-lvd1689m). The app enables TRELLIS only after a real GPU-generated GLB has passed validation. Images stay on this PC during 3D generation.

Install the optional GPU runtime with `powershell -ExecutionPolicy Bypass -File scripts/setup-trellis.ps1`. It uses Ubuntu 22.04 under WSL, a separate Python environment and CUDA 12.8; see the script's prerequisites. If DINOv3 access is pending, run `powershell -ExecutionPolicy Bypass -File scripts/setup-trellis.ps1 -VerifyOnly` after approval to finish the model download and generate a real verification GLB. Refresh the 3D studio to update its readiness indicator. Pending approval alone does not enable generation.
...
The gas lenses are real wavelength datasets, not colored copies of one image. Narrowband views include continuum and sometimes neighboring lines; gas isolation requires continuum subtraction and appropriate calibration. They are not abundance measurements or automatic gas identification.
